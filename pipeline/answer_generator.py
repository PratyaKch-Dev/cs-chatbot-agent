...
# ── Chitchat / missing-info template loader ───────────────────────────────────
_CHITCHAT_TEMPLATES_FILE = Path(__file__).parent.parent / "config" / "chitchat_templates.yaml"
_chitchat_templates: dict = {}


def _load_chitchat_templates() -> dict:
    global _chitchat_templates
    if not _chitchat_templates:
        try:
            _chitchat_templates = yaml.safe_load(
                _CHITCHAT_TEMPLATES_FILE.read_text(encoding="utf-8")
            ) or {}
        except Exception:
            _chitchat_templates = {}
    return _chitchat_templates


def get_chitchat_template(template_key: str, lang: str) -> str:
...
def generate_answer(
    ...
    # Pre-filled answer (e.g. troubleshooting template) — skip LLM entirely
    if prefilled_answer:
    ...
    # Chitchat — look up template from YAML by template_key (e.g. "greeting")
    if "CHITCHAT" in route.upper():
        key = template_key or intent
        text = get_chitchat_template(key, lang)
        if not text:
            text = get_chitchat_template("greeting", lang)  # safe fallback
        return GeneratedAnswer(
            text=text,
            grounding_score=1.0,
            was_escalated=False,
    ...
    # Missing info — look up "missing_info" template from same YAML
    if "MISSING_INFO" in route.upper():
        text = get_chitchat_template("missing_info", lang)
        return GeneratedAnswer(
            text=text,
            grounding_score=1.0,
            was_escalated=False,
    ...
    # High retrieval score (≥ 0.4) means the vector search found a strong match —
